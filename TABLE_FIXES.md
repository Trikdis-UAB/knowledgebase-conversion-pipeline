...
## Issue 7: Vista 48 Table - 3 Tables Displayed as 1 Wide Table

### Symptom
The "Special settings for Honeywell Vista 48 panel" table appears as a single 6-column table in markdown:
...
In the PDF, this appears as **3 separate 2-column tables side-by-side**, which is easier to read and matches the visual layout.

### Root Cause
The PDF layout uses 3 tables arranged horizontally across the page. Pandoc correctly converts this to a 6-column table with all data preserved, but the resulting wide table is harder to read than the original 3-table layout.

The "S" separator column was already being removed by `remove-empty-table-columns.lua`.

### Fix
**File**: `fix-vista48-table.py` (new Python script)

Created a custom post-processing script that:
1. Detects the Vista 48 section by heading marker
2. Finds the 6-column table between specific text markers
3. Splits each row into 3 separate 2-column rows
4. Rebuilds as 3 separate tables with proper headers

**Integration**: `convert-single.sh` (line 197)
```bash
...
**Result**: The table now appears as 3 separate 2-column tables matching the PDF layout:
...
**Benefits:**
- Matches the visual layout of the original PDF
- Easier to read and scan for specific settings
- Preserves all data exactly as in 6-column table
- Works automatically on every conversion
...
- (pending) - Split Vista 48 6-column table into 3 separate 2-column tables
